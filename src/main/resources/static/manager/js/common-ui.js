...
$(function() {
	 ...
	 * 
	 */
	$.initPageJQGrid = function(selector,param,size) {
		var defaultSetting = {
			url : '',
			mtype : "post",
		...
		var s = $.extend(true,defaultSetting,param);

		var dataGrid = $(selector).jqGrid(s);
		$.setGridSize(dataGrid,size.w,size.h);
		return dataGrid;
	}
	
	$.setGridSize = function(dataGrid,w,h){
		dataGrid.jqGrid('setGridWidth', w).jqGrid('setGridHeight',h);
	}
	
	$.reloadGrid= function(dataGrid,options){
		//{page:1,postData:_postData}
		dataGrid.setGridParam(options,true).trigger('reloadGrid');
	};
});